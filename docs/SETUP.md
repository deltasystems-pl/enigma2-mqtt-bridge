...
read-and-publish path here adds nothing measurable, and the box's web interface pays the same
price for its own screenshot page.
...
memory to begin with, choose `off`, or `interval` with a long period, and take a capture on demand
with `cmd/screenshot` when you actually want one.
