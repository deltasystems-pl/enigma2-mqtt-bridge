...
| `wol_arm` | `off` | Switch on the image's own Wake-on-LAN setting („Wake On LAN" in its expert settings) at every start. Only where the image has one: on a receiver without it the label says „Wake-on-LAN is not available on this receiver" and the setting does nothing. Switching it off never switches the image's setting off. Never writable over MQTT and not in `info.settings` — `info.wol` says what the receiver's switch actually is. No `ethtool` is run; see [ADR-0012](adr/0012-wake-on-lan-is-the-image-s-switch.md) |
