...
restarted is short of memory, set `screenshot` to `off` or to a long `interval` and take one on
demand with `cmd/screenshot`; see [SETUP.md](SETUP.md#what-a-screenshot-costs). Plugin loads that
