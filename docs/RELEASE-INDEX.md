...
- one index signed by the spare moves every reader that sees it to rank 2, and the main key is
  ...
  anything;
- a build that carries other keys (an acceptance build, below) remembers and silences only those
  keys and can never raise the rank the release keys are judged by;
...
verdict, and it is not a failure. A reader that stores its memory validates it when it loads it.
This is the spec's "ignored for good" rank rule written per key: for key sets that keep the rules
above it gives the same verdicts as a single stored rank carried across releases, and where a set
breaks them it keeps the safer one.
...
- **`publish-index.yml`** runs when `Release` succeeds for a `v*` tag pushed here (the version is
  ...
  - **sign** runs in the `release-signing` environment, which admits the `main` branch only and
    ...
    key's id, the next serial - signs it with `/usr/bin/openssl pkeyutl -sign -rawin -keyform DER
    -inkey /dev/stdin`, the secret (one line of base64 of the raw 32-byte seed) piped in as DER
    behind the fixed 16-byte PKCS#8 prefix, `/usr/bin/base64` and `/usr/bin/openssl` run by absolute
    path under `/usr/bin/env -u INDEX_SIGNING_KEY`, and nothing written to disk but the signature -
    then verifies the signature with the embedded main key before handing it on;
...
rule; in short: the one step that holds the key is pinned by the sha256 of its text and may run only
the emptiness test, `echo`, `exit`, `umask`, `printf` and the two programs by absolute path under
`/usr/bin/env -u INDEX_SIGNING_KEY`; the `secrets` context appears once in the whole repository;
only that job may name the `release-signing` environment; nothing in the sign job may touch
`GITHUB_PATH`, `GITHUB_ENV`, `BASH_ENV` or `PATH`, or declare `env` or `defaults`; every `uses:`,
flow style included, is pinned to a full commit SHA with its version beside it; no
`pull_request_target`; no `continue-on-error`, `||`, `set +e` or `always()` that would let a failed
guard pass; and every job of the chain is refused off `main` by a condition that nothing can
loosen.
...
summary - "Nothing is published yet: this is the first index", serial 1, key `5de3b24c97e88660`,
0.1.0 to 0.3.0 with 0.1.0 below the floor - approve sign, and do not start another run by hand; one
...
publish job and `index.yml` therefore read the branch's history and refuse, with a message naming
the commit that published the highest serial, when the published pair is older than that, signed by
a key an index published later silenced, or gone. **Recovery:** restore `feed/releases.json` and
`feed/releases.json.sig` from that commit, in a new commit, and run again.
